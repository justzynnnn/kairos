...
export async function POST(request: Request) {
  if (
    !(await allowPersistentRequest(
      clientKey(request.headers, "transcribe"),
      10,
    ))
  )
  ...
  if (!isOpenAIConfigured())
    return NextResponse.json(
      { error: "Add OPENAI_API_KEY to enable private voice transcription." },
      { status: 503 },
    );
  ...
  if (
    ...
    return NextResponse.json(
      { error: "Record a voice command smaller than 10 MB." },
      { status: 400 },
    );
  }
  ...
  if (
    ...
    duration < 1 ||
    duration > AI_LIMITS.maxRecordingSeconds
  ) {
    return NextResponse.json(
      {
        error: `Recordings are limited to ${AI_LIMITS.maxRecordingSeconds} seconds.`,
      },
      { status: 400 },
    );
  }
  ...
  if (!allowed)
    return NextResponse.json(
      { error: "Your daily voice transcription limit has been reached." },
      { status: 429 },
    );
  try {
    ...
    return NextResponse.json(
      {
        error:
          "Voice transcription is temporarily unavailable. Your recording was not saved.",
      },
      { status: 502 },
    );
  }
