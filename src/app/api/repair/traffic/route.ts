...
const schema = z.object({
  ...
  delayMinutes: z.number().int().min(5).max(360),
...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Choose a traffic delay between 5 minutes and 6 hours." },
      { status: 400 },
    );
  try {
    const now = new Date(),
      [viewer, items] = await Promise.all([getViewer(), getCalendarItems()]),
      item = items.find((entry) => entry.id === parsed.data.itemId);
    if (!item) throw new Error("Calendar item not found.");
    const predictedArrival = new Date(
        now.getTime() + parsed.data.delayMinutes * 60_000,
      ).toISOString(),
      repair = await repairTrafficDisruption({
        ...
        journey: {
          ...
          durationMinutes: parsed.data.delayMinutes,
          distanceMeters: 0,
          ...
          source: "seeded_demo",
          accuracyWarning: "User-reported traffic delay.",
        },
        journeySessionId: parsed.data.journeySessionId ?? randomUUID(),
      });
    return NextResponse.json({ repair });
  } catch (error) {
    return NextResponse.json(
      { error: userMessage(error, "Traffic repair could not be applied.") },
      { status: 422 },
    );
  }
}
