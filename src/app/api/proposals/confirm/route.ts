...
export async function POST(request: Request) {
  const parsed = confirmProposalSchema.safeParse(
    await request.json().catch(() => null),
  );
  if (!parsed.success)
    return NextResponse.json(
      { error: "The edited proposal is invalid." },
      { status: 400 },
    );
  ...
  try {
    ...
    const message =
      error instanceof SchedulingValidationError
        ? error.message
        : "The proposal no longer fits your calendar.";
    return NextResponse.json({ error: message }, { status: 409 });
  }

  if (!isSupabaseConfigured()) {
    const nextItems: CalendarItem[] = parsed.data.items.map((item) => ({
      id: randomUUID(),
      userId: viewer.id,
      type: item.type,
      title: item.title,
      ...
      normalDurationMinutes: item.normalDurationMinutes,
  ...
  const { data, error } = await supabase.rpc("confirm_schedule_proposal", {
    ...
    p_preferences: parsed.data.remember
      ? explicitPreferences(parsed.data.items)
      : [],
  });
  if (error) {
