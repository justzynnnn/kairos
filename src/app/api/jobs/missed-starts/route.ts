...
export async function GET(request:Request){
  ...
  const now=new Date(),lookback=new Date(now.getTime()-48*3600_000).toISOString();
  const {data,error}=await admin.from("calendar_items").select("user_id").eq("status","scheduled").not("start_at","is",null).gte("end_at",lookback).lte("end_at",now.toISOString()).limit(5000);
