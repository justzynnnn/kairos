...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Choose a valid Journey item." },
      { status: 400 },
    );
  try {
    ...
    if (
      ...
      throw new Error(
        "Choose a destination for a scheduled item before starting Journey.",
      );
    ...
    const session = await createHostedJourneySession(
      await createServerSupabaseClient(),
      item,
    );
    return NextResponse.json({
      session: { ...session, backgroundCapable: true },
    });
  } catch (error) {
    return NextResponse.json(
      { error: userMessage(error, "Journey could not be started.") },
      { status: 422 },
    );
  }
}
