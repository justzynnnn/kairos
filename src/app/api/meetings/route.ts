...
export async function POST(request: Request) {
  if (
    !(await allowPersistentRequest(clientKey(request.headers, "meetings"), 30))
  )
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Enter a meeting request under 1,000 characters." },
      { status: 400 },
    );
  try {
    const result = await createMeeting(parsed.data.command, request);
    if (result.status === "meeting")
      await recordMeetingActivity(result.meeting, "created");
    return NextResponse.json(result, {
      status: result.status === "impossible" ? 422 : 200,
    });
  } catch (error) {
    return NextResponse.json(
      {
