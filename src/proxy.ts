...
import { getSupabasePublicConfig } from "@/lib/env"; // These carry their own credentials: a hashed one-time booking token or CRON_SECRET.
const publicPaths = [
  "/auth",
  "/auth/callback",
  "/offline",
...
export async function proxy(request: NextRequest) {
  ...
  if (!c) {
    ...
    if (deployed && process.env.KAIROS_ALLOW_PREVIEW !== "1")
