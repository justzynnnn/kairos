...
import { AlertCircle, Check, LoaderCircle, Search, UserPlus, Users } from "lucide-react";
...
export function FriendSearch(){
  ...
  return <section className="card overflow-hidden" aria-labelledby="find-friends-title">
    <header className="border-b border-[var(--outline)] p-5">
      ...
      <p className="mt-3 text-sm text-[var(--muted)]">Search by name, username, or email, then send a private friend request.</p>
    </header>
    <div className="p-4 sm:p-5">
      <form onSubmit={search} role="search" className="flex gap-2">
        ...
        <button type="submit" disabled={searching||query.trim().length<2} className="inline-flex min-h-12 items-center justify-center rounded-xl bg-[var(--navy)] px-5 text-sm font-semibold text-white disabled:opacity-45">{searching?<LoaderCircle className="size-4 animate-spin"/>:"Search"}</button>
      </form>
