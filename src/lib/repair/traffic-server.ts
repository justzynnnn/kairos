...
export async function repairTrafficDisruption(context: {
  ...
  const { journey, journeySessionId, viewer, items, supabase } = context;
  if (journey.delayMinutes < 5) return null;
  ...
  if (!isSupabaseConfigured()) {
    const options = {
      ...
      latest = latestPreviewIncident({ ...options, includeResolved: true });
    if (latest && latest.delayMinutes + 10 > journey.delayMinutes)
      return latestPreviewIncident(options);
  }
  const request = trafficRepairRequest(
      journey.itemId,
      journey.predictedArrival,
      journey.delayMinutes,
    ),
    blockedUntil = new Date(journey.predictedArrival).getTime(),
    now = Date.now();
  request.travelBufferMinutes = viewer.travelBufferMinutes;
  request.requiresProtectedReview = items.some(
    (item) =>
      item.id !== journey.itemId &&
...
export async function repairBackgroundTrafficDisruption(context: {
  ...
  const { admin, tokenHash, session, viewer, items, journey } = context;
  if (
    journey.delayMinutes < 5 ||
    (session.lastTriggerDelay > 0 &&
      journey.delayMinutes < session.lastTriggerDelay + 10)
  )
    return null;
  const request = trafficRepairRequest(
      journey.itemId,
  ...
  if (solution.status === "impossible") {
    const { data, error } = await admin.rpc(
      "record_background_repair_attention",
      {
        p_token_hash: tokenHash,
        p_trigger: "traffic",
        p_reason: `${reason} ${solution.reason}`,
        p_delay_minutes: journey.delayMinutes,
    ...
    if (error || !data)
      throw new AppError(
        "The traffic disruption could not be recorded safely.",
      );
    return {
      id: String(data),
      trigger: "traffic",
  ...
  const { data: proposal, error: proposalError } = await admin
    ...
    .insert({
      ...
      payload: {
        trigger: "traffic",
        delay_minutes: journey.delayMinutes,
        blocked_until: journey.predictedArrival,
        anchor_item_id: journey.itemId,
  ...
  if (proposalError || !proposal)
    throw new AppError("The background repair proposal could not be saved.");
  ...
  if (error)
    throw new AppError("The background repair could not be applied safely.");
  const id =
    typeof data === "object" &&
    data &&
    "incident_id" in data &&
    (data as { incident_id: unknown }).incident_id
  ...
  const { data: row } = await admin
    ...
    .eq("id", id)
    .maybeSingle();
  if (!row || row.dismissed_at || row.status === "undone") return null;
  return {
    id: row.id,
    trigger: row.trigger,
    reason: row.reason,
    ...
    status: row.status,
    operations:
      (row.summary as { operations?: RepairIncident["operations"] } | null)
        ?.operations ?? [],
