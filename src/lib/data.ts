...
export const getViewer = cache(async (): Promise<Viewer> => {
  ...
  const { data: p } = await supabase
    .from("profiles")
    .select(
      "full_name,username,timezone,active_start,active_end,travel_buffer_minutes,avatar_url,schedule_version",
    )
    .eq("id", data.user.id)
    .single();
  return {
    ...
    activeEnd: p?.active_end ?? "22:30",
    travelBufferMinutes: p?.travel_buffer_minutes ?? 15,
    avatarUrl: p?.avatar_url ?? null,
    preview: false,
    scheduleVersion: p?.schedule_version ?? 1,
  };
});
export const getCalendarItems = cache(async (): Promise<CalendarItem[]> => {
  if (!isSupabaseConfigured()) return getDemoCalendarItems();
  const supabase = await createServerSupabaseClient(); // Deadlines carry a null start_at, so they are kept alongside the recent window.
  const cutoff = new Date(
    Date.now() - CALENDAR_WINDOW_DAYS * 86_400_000,
  ).toISOString();
  const [{ data, error }, { data: dependencies, error: dependencyError }] =
    await Promise.all([
      supabase
        .from("calendar_items")
        .select("*")
        ...
        .limit(CALENDAR_ITEM_LIMIT),
      supabase
        .from("calendar_item_dependencies")
        .select("item_id,depends_on_id"),
    ]);
  if (error || dependencyError)
    throw new AppError("Unable to load your schedule.");
...
export const getPreferences = cache(async (): Promise<Preference[]> => {
  ...
  if (error) throw new AppError("Unable to load preferences.");
  return (data ?? []).map((row) => ({
    id: String(row.id),
