// Local preview and tests intentionally remain isolated from hosted state.
...
export function allowRequest(
  ...
  if (!bucket) {
    if (buckets.size >= MAX_TRACKED) evictStale(now, windowMs);
    buckets.set(key, { tokens: limit - 1, stamp: now });
    return true;
  }
...
function evictStale(now: number, windowMs: number) {
  for (const [key, bucket] of buckets)
    if (now - bucket.stamp > windowMs * 2) buckets.delete(key);
}
